...
export function About(): JSX.Element {
  ...
  return (
    <section
      id="about"
      className="border-b border-slate-200 bg-canvas py-20 lg:py-28"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {HIGHLIGHTS.map((item, index) => {
              ...
              return (
                <Reveal key={item.id} delayMs={120 + index * 80} className="h-full">
                  <article className="h-full rounded-xl border border-slate-200/80 bg-white p-6 shadow-panel sm:p-8">
        ...
        <Reveal delayMs={160}>
          <div className="mt-14 rounded-2xl border border-slate-200/90 bg-gradient-to-b from-white to-slate-50/70 p-6 shadow-panel sm:p-8 lg:p-10">
            <div className="mb-8 flex flex-col gap-2 border-b border-slate-200/80 pb-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              {CONTACT_PEOPLE.map((person) => (
                <div
                  key={person.id}
                  className="group relative flex flex-col gap-5 rounded-xl border border-slate-200/90 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-amber-500/60 hover:shadow-xl sm:flex-row sm:items-start"
                >
                  {person.photo ? (
                    <div className="relative mx-auto h-32 w-32 shrink-0 overflow-hidden rounded-2xl border-2 border-amber-500/30 bg-slate-100 shadow-md ring-4 ring-amber-500/10 transition-transform duration-500 group-hover:scale-105 sm:mx-0 sm:h-36 sm:w-36">
                      <Image
                        src={person.photo}
                        alt={person.name}
                        fill
                    ...
                    <span
                      aria-hidden
                      className="flex h-32 w-32 shrink-0 items-center justify-center rounded-2xl bg-amber-50 text-amber-700 sm:h-36 sm:w-36"
                    >
                  ...
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <h4 className="mt-1.5 text-xl font-bold tracking-tight text-ink sm:text-2xl">
                      {person.name}
                    </h4>
